...
uses
  ...
  ExtCtrls, AppEvnts, JclSysUtils, JclMapi, JclUnitVersioning, JclUnitVersioningProviders,
  JclDebug, JvExControls, JvLabel, RzCommon, RzPanel;
...
type
  TExceptionDialog = class(TForm)
    ...
    pnTopo: TRzPanel;
    pnSepTopo: TRzPanel;
    pnSepTextMemo: TRzPanel;
    rzfc_Controles: TRzFrameController;
...
resourcestring
  RsAppError = '%s - ERRO NO PROGRAMA';
  RsExceptionClass = 'Classe do Erro: %s';
  RsExceptionMessage = 'mensagem de Erro: %s';
  RsExceptionAddr = 'endereço do Erro: %p';
  RsStackList = 'Lista de Erros, Gerado em %s';
  RsModulesList = 'Lista de Módulos Carregados:';
  RsOSVersion = 'Sistema   : %s %s, Versão: %d.%d, Compilação: %x, "%s"';
  RsProcessor = 'Processador: %s, %s, %d MHz';
  RsMemory = 'Memória: %d; Livre %d';
  RsScreenRes = 'Resolução  : %dx%d Pixels, %d bpp';
  RsActiveControl = 'Hierarquia de Controles Ativos:';
  RsThread = 'Thread: %s';
  RsMissingVersionInfo = '(Sem Informações da versão do Módulo)';
  RsExceptionStack = 'Excessão da Pilha';
  RsMainThreadID = 'Thread Principal: ID = %d';
  RsExceptionThreadID = 'Thread do Erro: ID = %d';
  RsMainThreadCallStack = 'Pilha de Chamadas da Thread Principal';
  RsThreadCallStack = 'Pilha Carregada Pela Thread %d %s "%s"';
  RsExceptionThreadCallStack = 'Pilha de chamadas para a linha de exceção %s';
  RsErrorMessage = 'Houve um erro durante a execução deste programa.' + NativeLineBreak +
                   'O aplicativo pode se tornar instável e até mesmo travar.' + NativeLineBreak +
                   'É recomendado que você salve seu trabalho e feche esta aplicação.' + NativeLineBreak + NativeLineBreak;
  RsDetailsIntro = 'Log Técnico com as informaçãos detalhadas da Exceções, Gerada em %s.' + NativeLineBreak +
                   'Você pode enviá-lo para o fornecedor do aplicativo, ajudando-o a entender o que aconteceu.' + NativeLineBreak +
                   ' Título do Aplicativo: %s' + NativeLineBreak +
                   ' Arquivo da Aplicação: %s';
  RsUnitVersioningIntro = 'Arquivo de Informações de Versionamento:';
...
begin
  ...
  try
    ...
    Recipients.Add('gerson@casasistemas.com.br');
...
begin
  ...
  try
    DefaultExt := '.log';
    FileName := ExtractFileName(Application.ExeName) + '-Erro-' + FormatDateTime('yyyy-mm-dd', Date) + '.log';
    Filter := 'Arquivos de Log (*.log)|*.log|All files (*.*)|*.*';
